...
class FormularioLivro(FlaskForm):
    ...
    categoria = StringField("Categoria", [validators.data_required(), validators.Length(min=1, max=30)])
...
class FormularioLoginUsuarios(FlaskForm):
    ra = IntegerField("RA", [validators.data_required(), validators.NumberRange(min=1, max=9999999)])
    senha = PasswordField("Senha", [validators.data_required(), validators.Length(min=1, max=100)])
    login = SubmitField("Login")
...
class FormularioCadastroUsuarios(FlaskForm):
    ...
    ra = IntegerField("Registro Acadêmico (RA)", [validators.data_required(), validators.NumberRange(min=5, max=9999999)])
    email = EmailField("Email", [validators.data_required(), validators.Email(), validators.Length(min=5, max=100)])
    senha = PasswordField("Senha", [validators.data_required(), validators.Length(min=5, max=100)])
...
def usuario_pode_reservar(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not usuario_atual or not usuario_atual.pode_reservar:
            ...
            return redirect(url_for("lista_tabelas"))
        return func(*args, **kwargs)
